...
class MissionUpdatePage extends StatefulWidget {
  final Mission oldMission;
  final int missionNumber;
  const MissionUpdatePage({super.key, required this.oldMission, required this.missionNumber});

  @override
  _MissionUpdateState createState() => _MissionUpdateState();
}

class _MissionUpdateState extends State<MissionUpdatePage> {
  // 状態を管理する変数
  late int missionNumber;
  Mission? oldMissionData;
  bool loading = false;
  ...
  void initState() {
    super.initState();
    oldMissionData = widget.oldMission;
    missionNumber = widget.missionNumber;
    missionTextController = TextEditingController(text: oldMissionData?.missionText);
    rewardController = TextEditingController(text: oldMissionData?.reward);
    penaltyController = TextEditingController(text: oldMissionData?.penalty);
    opportunityController = TextEditingController(text: oldMissionData?.opportunity);
    noteController = TextEditingController(text: oldMissionData?.note);
    DateTime dt = DateTime.parse(oldMissionData!.starTime.substring(0,10)+" "+oldMissionData!.starTime.substring(11,16));
    _selectedStartDate = DateTime(dt.year, dt.month, dt.day);
  ...
  void _showRepeatSettingModal(BuildContext context) async {
    final result = await showModalBottomSheet<RepeatSettingData>(
      ...
      builder: (BuildContext context) {
        return RepeatSetting(oldMission: oldMissionData!);
      },
    );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        appBar: AppBar(title: const Text('ミッションの更新')),
...
class RepeatSetting extends StatefulWidget {
  final Mission oldMission;
  const RepeatSetting({super.key, required this.oldMission});
...
class _RepeatSettingState extends State<RepeatSetting> {
  ...
  void initState() {
    super.initState();
    oldMissionData = widget.oldMission;
    if(oldMissionData!.repeatType!=0) _isRepeated=true;
    _repeatInterval = oldMissionData!.repeatInterval;
    if(oldMissionData!.repeatType==1){
      _repeatOption = '日';
    }else if(oldMissionData!.repeatType==2){
      _repeatOption = '週';
